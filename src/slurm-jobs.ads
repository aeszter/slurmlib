...
--  slurm_complete_job - Note completion of a job. Releases resource allocation for the job.
   ...
   function First (Collection : List) return Iterator;
   function Last (Collection : List) return Iterator;
   procedure Next (Position : in out Iterator);

   function Get_ID (Position : Iterator) return Positive;

   procedure Load_Jobs (Destination : in out List);
...
private
   ...
   type Iterator is record
      Container : job_info_msg_ptr;
      Position : job_info_ptr;
      Last : job_info_ptr;
   end record;

   type List is new Ada.Finalization.Limited_Controlled with record
      List_Ptr : job_info_msg_ptr_ptr;
   end record;
